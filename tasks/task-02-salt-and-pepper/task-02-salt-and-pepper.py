...
def create_salt_and_pepper_noise(height=100, width=100, salt_prob=0.05, pepper_prob=0.05):
    ...
    """
    ### START CODE HERE ###
    img = np.full((height, width), 0.5)  # Inicializa com 0.5
    rand = np.random.rand(height, width)  # Gera valores aleatórios entre 0 e 1
    img[rand < salt_prob] = 1.0  # Sal
    img[(rand >= salt_prob) & (rand < salt_prob + pepper_prob)] = -1.0  # Pimenta
    ### END CODE HERE ###
    return img
